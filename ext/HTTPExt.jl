...
import Treebars: ws_progress, ProgressNode, StateProgress, progress_state

"""
    ws_progress(ws, node; interval=0.1, render=default_render)

Push live progress updates over a WebSocket until the node finalizes.

By default sends `repr(progress_state(node))`. The HTMXObjectsExt overrides
...
"""
ws_progress(ws::WebSocket, node::ProgressNode{<:StateProgress};
        interval=0.1, render=node -> repr(progress_state(node))) = begin
